...
import (
	...
	"github.com/s4ch/CloakRipper/internal/config"
	"github.com/s4ch/CloakRipper/internal/dns"
	httpClient "github.com/s4ch/CloakRipper/internal/http"
	"github.com/s4ch/CloakRipper/pkg/models"
...
func (s *Scanner) securityTrailsSearch(domain, url string, cfIP net.IP, actualHTMLTitle string) {
	...
	if len(keys) == 0 {
		fmt.Println("[!] SecurityTrails API key not found in ~/.config/CloakRipper.yaml")
		return
	}
	key := keys[0]
	if key == "" {
		fmt.Println("[!] SecurityTrails API key is empty in ~/.config/CloakRipper.yaml")
		return
	}
...
func (s *Scanner) shodanSearch(domain, url string, cfIP net.IP, actualHTMLTitle string) {
	...
	if len(keys) == 0 {
		fmt.Println("[!] Shodan API key not found in ~/.config/CloakRipper.yaml")
		return
	}
	key := keys[0]
	if key == "" {
		fmt.Println("[!] Shodan API key is empty in ~/.config/CloakRipper.yaml")
		return
	}
